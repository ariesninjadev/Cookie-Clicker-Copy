...
var OldPlaySound=function(url,vol)
{
	...
	if (!Game.volume || volume==0 || true) return 0;
...
Game.Launch=function()
{
	...
	Game.ErrorFrame=function()
	{
		...
		'<div class="title">Wrong webpage!</div>'+
		'<div>It looks like you\'re accessing Cookie Clicker from another URL than Aries\' copy.<br>'+
		'You can <a href="https://ccc.ariesninja.repl.co/" target="_blank">play Cookie Clicker over here</a>!<br>'+
		'<small>(If for any reason, you are unable to access the game on the official URL, you can play on our copy.)</small></div>';
	}
	Game.timedout=false;
	...
	Game.Init=function()
	{
		...
		Game.bakeryNameRefresh=function()
		{
			...
			if (!App && name.indexOf('sayswhatthesigma',name.length-('sayswhatthesigma').length)>0 && !Game.sesame) Game.OpenSesame();
			if (!App && name.indexOf('saysopensesame',name.length-('saysopensesame').length)>0 && !Game.sesame) alert("Well that would ruin the fun, wouldn't it?");
		...
		if (!App)
		{
			Game.attachTooltip(l('httpsSwitch'),'<div style="padding:8px;width:350px;text-align:center;font-size:11px;">'+loc("You are currently playing Cookie Clicker on Aries' private server.<br>The <b>public</b> version may not be available.",[(Game.https?'HTTPS':'HTTP'),(Game.https?'HTTP':'HTTPS')])+'</div>','this');
		...
		Game.ExportSave=function()
		{
			...
			Game.Prompt('<id ExportSave><h3>'+loc("Export save")+'</h3><div class="block">'+loc("This is your save code.<br>Copy it and keep it somewhere safe!<br>THIS CODE WILL WORK ON THE ORIGINAL SITE!")+'</div><div class="block"><textarea id="textareaPrompt" style="width:100%;height:128px;" readonly>'+Game.WriteSave(1)+'</textarea></div>',[loc("All done!")]);//prompt('Copy this text and keep it somewhere safe!',Game.WriteSave(1));
		...
		//Game.last.minigameUrl='dungeons.js';//not yet
